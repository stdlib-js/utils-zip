...
function zip() {
	...
	for ( i = 0; i < nargs-1; i++ ) {
		if ( !isArray( args[i] ) ) {
			throw new TypeError( format( '1YhBw', args[i] ) );
		}
	}
	arg = args[ nargs-1 ];
	flg = isObject( arg );
	if ( !flg && !isArray( arg ) ) {
		throw new TypeError( format( '1YhBx', arg ) );
	}
	if ( flg ) {
		opts = args.pop();
	}
	nargs = args.length;
	if ( nargs === 0 ) {
		throw new Error( format('1Yh1x') );
	}
	if ( hasOwnProp( opts, 'trunc' ) ) {
		if ( !isBoolean( opts.trunc ) ) {
			throw new TypeError( format( '1Yh2o', 'trunc', opts.trunc ) );
		}
	} else {
		opts.trunc = true;
	}
	...
	if ( hasOwnProp( opts, 'arrays' ) ) {
		if ( !isBoolean( opts.arrays ) ) {
			throw new TypeError( format( '1Yh2o', 'arrays', opts.arrays ) );
		}
	} else {
		opts.arrays = false;
	}
